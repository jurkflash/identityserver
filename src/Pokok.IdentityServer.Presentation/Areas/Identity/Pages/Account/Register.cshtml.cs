...
namespace Pokok.IdentityServer.Presentation.Areas.Identity.Pages.Account
{
    ...
    {
        private readonly SignInManager<PokokUser> _signInManager;
        ...
        private PokokUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<PokokUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(PokokUser)}'. " +
                    $"Ensure that '{nameof(PokokUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
